...
let viz7_gendata fname =
  ...
  let read_age_and_cod_from_file year =
    ...
    yr |> match year with
    | y when range (1968,1978) y -> get (38,3,icd8_9_age) (74,3,nchs_code_to_name nchs.recode34)
    | y when range (1979,1998) y -> get (63,3,icd8_9_age) (156,3,nchs_code_to_name nchs.recode34)
    | y when range (1999,2002) y -> get (63,3,icd8_9_age) (156,2,nchs_code_to_name nchs.recode39)
    | y when range (2003,2014) y -> get (69,4,icd10_age) (159,2,nchs_code_to_name nchs.recode39)
    | _ -> failwith "bad year" in
  ...
  let generate_age_counts =
    List.map (fun (cod,ages) -> (cod, list_from_hash 121
      (fun tbl -> List.iter (inc_table tbl (fun c -> c+1) (fun () -> 1)) ages))) in
  ...
  let remove_invalid_ages =
    List.map (fun (cod,ages) -> (cod,
      List.fold_left (fun acc (age,ct) -> (* can't trust ages over 120, wiki says there aren't any from the US *)
        if age<0 || age>120 then acc else (age,ct)::acc) [] ages)) in

  let group_by_cause yrs =
    list_from_hash 25 (fun tbl ->
      List.iter (fun yr ->
        List.iter (fun (cod,agects) ->
          inc_table tbl (fun agectsl -> agects @ agectsl) (fun () -> agects) cod) yr) yrs) in

  let sum_years causes =
    List.map (fun (cod,agecountlist) -> (cod,list_from_hash 121 (fun tbl ->
      List.iter (fun (age,count) ->
        inc_table tbl (fun ct -> ct+count) (fun () -> count) age) agecountlist))) causes in

  let normalize_counts =
    List.map (fun (cod,agects) ->
      let total = List.fold_left (fun acc (age,ct) -> acc+ct) 0 agects |> foi in
      (cod, List.map (fun (age,ct) -> (age, (foi ct)/.total)) agects)) in

  let sort_counts =
    List.map (fun (cause,ages) -> (cause, List.sort (fun (a1,_) (a2,_) -> a1-a2) ages)) in

  let fill_missing_years =
    List.map (fun (cause,ages) -> (cause, List.fold_left (fun acc age ->
      if List.exists (fun (a1,_) -> a1=age) acc then acc else (age,0.0)::acc) ages (0|..|120))) in
  ...
  let pmfs_to_json pmfs = (* pmf representation list of form [(name,[(age,prob)])] to json *)
    `Dict (List.map (fun (pmf_name,pts) -> (pmf_name,
      `List (List.map (fun (age,prob) ->
        `Dict [("age",`Int age);("percentage",`Float prob)]) pts))) pmfs) in
  ...
  |> List.map (fun y -> y
    ...
    |> generate_age_counts
    |> remove_invalid_ages
    |> pass (fun _ -> printf "finished preprocessing year %d\n" y; flush Pervasives.stdout))
  |> pass (fun _ -> printf "passed the initial read and map\n%!")
  |> group_by_cause
  |> sum_years
  |> normalize_counts
  |> fill_missing_years
