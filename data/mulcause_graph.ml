...
let viz1_gendata fname =
  ...
  let start_icd,len_icd = 145,4 in
  let start_multiple,len_multiple = 166,138 in
   ...
   * the disease type *)
  let icd_category_map = Hashtbl.create nchs.icd_length in
  ...
  let lookup_fun range name =
    fun num -> if cause_range range num then Some name else None in
  ...
  let disease_type tag =
    ...
    with _ -> failwith (sprintf "%s" (String.sub tag 1 (String.length tag-1))) in
    ...
    |> maptr (fun l ->
        (String.sub l start_icd len_icd (* cause *)
         ...
         String.sub l start_multiple len_multiple (* multiple conditions *)
          |> Str.split (Str.regexp " +\([0-9][0-9]\)?"))) in

  (* transform an mc list to names *)
  let mulcause_to_humanized mc_data =
    maptr (fun (cause,multiple) -> (disease_type cause, maptr disease_type multiple)) mc_data in
  ...
  let humanized_to_graph humanized_data =
    ...
    Hashtbl.fold (fun cond (ct,preex_ht) acc -> (cond,ct,loh preex_ht) :: acc) nodes [] in
