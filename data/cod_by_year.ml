...
let viz2_gendata (* timeseries_1968_1998 *) fname =
  ...
  let icd8_start,icd8_len = 74,3 in
  let icd9_start,icd9_len = 156,3 in
  let icd10a_start,icd10a_len = 156,2 in
  let icd10b_start,icd10b_len = 159,2 in
  ...
  (* good for 68 - 78 *)
  let get_icd8_causes = get_icdX_causes nchs.recode34 (icd8_start,icd8_len) in
  (* good for 79 - 98 *)
  let get_icd9_causes = get_icdX_causes nchs.recode34 (icd9_start,icd9_len) in
  (* good for 99 - 14 *)
  let get_icd10a_causes = get_icdX_causes nchs.recode39 (icd10a_start,icd10a_len) in
  let get_icd10b_causes = get_icdX_causes nchs.recode39 (icd10b_start,icd10b_len) in
  ...
  let normalize_yearly_causes yearly_causes =
    List.map (fun (year,causes) ->
      ...
      (year, List.map (fun (cause,count) -> (cause, (foi count) /. total)) causes)) yearly_causes in
  ...
  let yearly_causes_to_timeseries yearly_cause_data =
    list_from_hash nchs.length
      ...
      ) yearly_cause_data) in


  let timeseries_to_json ts =
    `Dict (List.map (fun (cause,years) ->
      (cause, `List (List.map (fun (yr,pct) -> `List [`Int yr; `Float pct]) years))) ts) in
